...
def align_virtual_nodes(
    e_nodes, 
    q_sample_obs_norm,   # <--- The Raw Detector Peaks (Ground Truth)
    ...
    for idx, e_angle in enumerate(emp_pair_angles):
        ...
        for m in range(len(matches)):
            ...
            for s1, s2 in signs:
                # Order 1 (e1 -> s1*t1, e2 -> s2*t2)
                w1 = s1 * t1
                w2 = s2 * t2
                w2_ortho = np.cross(w1, w2); w2_ortho /= np.linalg.norm(w2_ortho)
                w3 = np.cross(w1, w2_ortho)
                U_hypotheses.append(V @ np.column_stack([w1, w2_ortho, w3]).T)
                
                # Order 2 (e1 -> s1*t2, e2 -> s2*t1)
                w1_f = s1 * t2
                w2_f = s2 * t1
                w2_f_ortho = np.cross(w1_f, w2_f); w2_f_ortho /= np.linalg.norm(w2_f_ortho)
                w3_f = np.cross(w1_f, w2_f_ortho)
                U_hypotheses.append(V @ np.column_stack([w1_f, w2_f_ortho, w3_f]).T)
    ...
    # SCORES U-MATRIX DIRECTLY AGAINST RAW PHYSICAL PEAKS
    @jit
    def evaluate_peaks_hard(U_batch_j, q_obs_j, r_eval_j, tol_deg):
        ...
        # jnp.abs() is mathematically required because Laue rays are unpolarized lines
    ...
    chunk_size = 1000 # Keep VRAM perfectly safe
    best_U = None
    best_inliers = -1
