...
def apply_hessian_starburst_filter(H_grid):
    ...
    # --- 2. The Spherical Metric Correction ---
    # Create an array of the corresponding Theta angles for the rows
    thetas = jnp.linspace(0, jnp.pi, H_smooth.shape[0])
    # Avoid division by zero exactly at the poles
    sin_theta = jnp.maximum(jnp.sin(thetas), 1e-4)[:, None]

    # Scale the azimuthal gradient by 1/sin(Theta) to recover true physical curvature
    dx = dx_raw / sin_theta
    dy = dy_raw # Theta gradient is physically uniform

    # --- 3. Compute Second Derivatives with Metric ---
    dyy, dyx_raw = jnp.gradient(dy)
    dxy_raw, dxx_raw = jnp.gradient(dx)

    dxx = dxx_raw / sin_theta
    dxy = dxy_raw / sin_theta

    # --- 4. True Spherical Topology ---
    det = (dxx * dyy) - (dxy ** 2)
    trace = dxx + dyy
